...
export class SearchController {
  ...
  async getSearchResults(req: Request, res: Response) {
    try {
      ...
      if (brand_id) {
        productQuery.andWhere("product.brand_id = :brand_id", {
          brand_id: Number(brand_id),
        });
      }
      ...
      if (!isNaN(minPrice) && !isNaN(maxPrice)) {
        ...
        productQuery.andWhere("product.price >= :minPrice", { minPrice });
      } else if (!isNaN(maxPrice)) {
        productQuery.andWhere("product.price <= :maxPrice", { maxPrice });
      }
  
      if (rating) {
        const ratingValue = parseInt(rating as string, 10);
        productQuery.having("AVG(review.rating) >= :rating", { rating: ratingValue });
      }
      ...
      if (latest_arrival === "desc") {
        ...
        productQuery.orderBy("product.price", "ASC");
      } else if (price === "highest") {
        productQuery.orderBy("product.price", "DESC");
      }
  
      if (top_rated === "top") {
        productQuery.having("AVG(review.rating) = 5.0");
      }
  
      if (popular === "popular") {
        productQuery.orderBy("COUNT(review.id)", "DESC");
      }
  
      // Additional filters
      if (more_suitable === "suitable") {
        productQuery.andWhere("product.discount IS NOT NULL AND product.discount > 0");
      }
      ...
      const totalCount = await productQuery.getCount();
  
      // Apply pagination
      productQuery.skip((Number(page) - 1) * Number(limit)).take(Number(limit));
      ...
      //       const averageRating =
      //         totalComments > 0
      //           ? reviews.reduce((sum, review) => sum + review.rating, 0) / totalComments
      //           : 0;
